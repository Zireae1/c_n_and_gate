...
for (file in flist[1:4]) {
  print(file)
  cairo_pdf(paste("graphs/", gsub(".txt", "", file), "_plot.pdf", sep=""), 
            width = 8, height = 8) # save plot
  plot_network(paste("data/", file, sep=""), layout=layout_nicely) # default layout is bipartite
  dev.off()
}
...
links1<-fread("data/Network_5_5_32_2_alt1.txt", header = F, stringsAsFactors = F)
links2<-fread("data/Network_5_5_32_2_alt2.txt", header = F, stringsAsFactors = F)
...
### plot network
cairo_pdf(paste("graphs/", "Network_5_5_32_2_alt1_alt2_plot.pdf", sep=""), width = 6, height = 6)
